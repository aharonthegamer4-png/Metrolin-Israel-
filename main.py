...
# מזהה החדר החדש שביקשת לאישורי בקשת רולים
ROLE_REVIEW_CHANNEL_ID = 1519040651130568754
@tasks.loop(minutes=1)
async def update_bot_status():
    """עדכון הסטטוס של הבוט לפי כמות המשתמשים שבאמת מחוברים כרגע בדיסקורד (בדיוק כמו בתמונה!)"""
    try:
        total_members = 0
        online_members = 0
...
class AcceptanceModal(discord.ui.Modal, title="טופס בדיקה וקבלת רולים למשתמש"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        review_channel = interaction.guild.get_channel(ROLE_REVIEW_CHANNEL_ID)
        
        if not review_channel:
            await interaction.response.send_message("שגיאה: חדר אישורי הנהלה החדש לא נמצא בשרת!", ephemeral=True)
            return
        ...
        # בניית חדר הבדיקה הנסתר: ההודעה נשלחת יחד עם רכיבי הדרופדאון והכפתורים המשולבים
...
class CrimeRoleDropdown(discord.ui.Select):
    def __init__(self, target_member: discord.Member, parent_view: AcceptanceActionView):
        ...
        options = [
            ...
            discord.SelectOption(label="👔 NON CRIME STAFF", value="1514624993886208201"),
        ...
        super().__init__(
            placeholder="בחר דרגות ורולי פשע להענקה (סמן כמה שצריך)...",
            min_values=1,
    ...
    async def callback(self, interaction: discord.Interaction):
        self.parent_view.selected_roles_pool = self.values
        await interaction.response.send_message("📢 הדרגות סומנו זמנית במערכת! לחץ למעלה על כפתור הסיום הירוק כדי לאשר ולסגור.", ephemeral=True)
...
class MafiaPanelLaunchView(discord.ui.View):
    ...
    @discord.ui.select(
        placeholder="בחר סוג הזמנת מאפיה...",
        custom_id="mafia_ticket_panel_select_fixed_v9",
        options=[
            discord.SelectOption(label="הזמנת נשקים חמים", value="weapons", description="פתיחת פנייה לרכישת נשק ותחמושת", emoji="🔫"),
            discord.SelectOption(label="הזמנת סמים / חומרים", value="drugs", description="פתיחת פנייה לרכישת סחורה לא חוקית", emoji="🌿"),
            discord.SelectOption(label="אחר / בירור מאפיה", value="other", description="נושאים כלליים הקשורים למאפיה", emoji="💼")
    ...
    @discord.ui.button(label="⚙️ ניהול ועדכון מלאי (צוות בלבד)", style=discord.ButtonStyle.secondary, custom_id="mafia_owner_manage_stock_fixed_v9", row=1)
