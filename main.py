...
# מזהה החדר החדש והקבוע שביקשת לאישורי בקשת רולים שלא יימחק לעולם
ROLE_REVIEW_CHANNEL_ID = 1519045095200915546
@tasks.loop(minutes=1)
async def update_bot_status():
    """עדכון הסטטוס של הבוט לפי כמות המשתמשים שבאמת מחוברים כרגע בדיסקורד"""
    try:
        total_members = 0
        online_members = 0
...
class AcceptanceActionView(discord.ui.View):
    ...
    async def select_roles_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("✅ הרולים הוענקו והטופס טופל בהצלחה!", ephemeral=True)
        
        # השבתת הכפתורים של הודעה זו בלבד כדי שלא ילחצו שוב, מבלי למחוק את הערוץ!
        for item in self.children:
            item.disabled = True
        await interaction.message.edit(view=self)
