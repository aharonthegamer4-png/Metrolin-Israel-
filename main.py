...
# מערכת זיכרון חכמה: שמירת קשר בין הודעות/ערוצים לבין המשתמשים ב-100% הצלחה
USER_FORMS = {}    # מקשר בין Message ID ל-User ID של מגיש הטופס
USER_TICKETS = {}  # מקשר בין Channel ID ל-User ID של פותח הטיקט
...
class AcceptanceActionView(discord.ui.View):
    ...
    def get_target_id_from_memory(self, message_id: int) -> int:
        """חילוץ הרמטי של האיידי ישירות מזיכרון הבוט ללא תלות בטקסט"""
        return USER_FORMS.get(message_id, 0)
    ...
    async def kick_user_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        target_id = self.get_target_id_from_memory(interaction.message.id)
        if target_id == 0:
            await interaction.response.send_message("❌ שגיאה: לא הצלחתי למצוא את המשתמש בזיכרון! אנא פתח טופס חדש.", ephemeral=True)
            return
        ...
        if not target_member:
            ...
            return
        try:
            await target_member.kick(reason="נדחה במערכת בדיקת משתמש")
    ...
    async def ban_user_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        target_id = self.get_target_id_from_memory(interaction.message.id)
        if target_id == 0:
            await interaction.response.send_message("❌ שגיאה: לא הצלחתי למצוא את המשתמש בזיכרון!", ephemeral=True)
            return
        try:
    ...
    @discord.ui.button(label="🛡️ סיום פנייה והענקת רולים (Approve)", style=discord.ButtonStyle.success, custom_id="accept_approve_fixed_final", row=0)
    async def select_roles_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not self.selected_roles_pool:
            await interaction.response.send_message("❌ לא סומן שום רול בדרופדאון למטה! אנא בחר רולים תחילה.", ephemeral=True)
            return

        added_names = []
        for role_id in self.selected_roles_pool:
        ...
        log_channel = interaction.guild.get_channel(1510196539962818653)
        if log_channel:
            embed = discord.Embed(
                title="✅ חבר שרת אושר בהצלחה!",
                description=f"**המשתמש:** {target_member.mention}\n**אושר על ידי:** {interaction.user.mention}\n**הרולים שהוענקו:** {', '.join(added_names) if added_names else 'ללא'}",
                color=discord.Color.green()
            )
            await log_channel.send(embed=embed)

        await interaction.response.send_message("✅ הרולים הוענקו והפנייה נסגרה בהצלחה!", ephemeral=True)
        await asyncio.sleep(2)
        await interaction.channel.delete()
class CrimeRoleDropdown(discord.ui.Select):
    def __init__(self, target_member: discord.Member, parent_view: AcceptanceActionView):
        self.parent_view = parent_view
        
        options = [
            discord.SelectOption(label="בעלים", value="1510196537874190401", emoji="🎮"),
            discord.SelectOption(label="צוות", value="1518269453295685652", emoji="🛠️"),
            discord.SelectOption(label="אחראי פשע", value="1510196537874190400", emoji="🔻"),
            discord.SelectOption(label="אבי", value="1518267524050063440", emoji="🍃"),
            discord.SelectOption(label="צוות הנהלה", value="1510196537874190399", emoji="🔮"),
            discord.SelectOption(label="NON CRIME STAFF", value="1514624993886208201", emoji="👔"),
            discord.SelectOption(label="הבורר", value="1510196537852956691", emoji="🕴"),
            discord.SelectOption(label="יד ימין - בוררות", value="1510196537852956690", emoji="🌌"),
            discord.SelectOption(label="יועץ - בוררות", value="1510196537852956689", emoji="🌌"),
            discord.SelectOption(label="אחראי חיילים - בוררות", value="1510196537852956688", emoji="🌌"),
            discord.SelectOption(label="חייל בכיר - בוררות", value="1510196537852956687", emoji="🌌"),
            discord.SelectOption(label="חייל - בוררות", value="1510196537852956686", emoji="🌌"),
            discord.SelectOption(label="אחראי שומרים - בוררות", value="1510196537852956685", emoji="🌌"),
            discord.SelectOption(label="שומר - בוררות", value="1510196537852956684", emoji="🌌"),
            discord.SelectOption(label="אימפריית הבוררות", value="1510196537852956683", emoji="🌌")
        ]
        
        super().__init__(
            placeholder="בחר דרגות ורולי בוררות/צוות (סמן כמה שצריך)...",
            min_values=1,
            max_values=len(options),
            options=options,
            custom_id="crime_role_select_pt1",
            row=1
        )

    async def callback(self, interaction: discord.Interaction):
        current_pool = list(self.parent_view.selected_roles_pool)
        for val in self.values:
            if val not in current_pool:
                current_pool.append(val)
        self.parent_view.selected_roles_pool = current_pool
        await interaction.response.send_message("📢 דרגות סומנו זמנית! ניתן לבחור עוד רולים מהתפריט השני או ללחוץ סיום למעלה.", ephemeral=True)
class CrimeRoleDropdownPartTwo(discord.ui.Select):
    def __init__(self, target_member: discord.Member, parent_view: AcceptanceActionView):
        self.parent_view = parent_view
        
        options = [
            discord.SelectOption(label="ראש ארגון", value="1510196537832116233", emoji="🌌"),
            discord.SelectOption(label="יד ימין", value="1510196537832116232", emoji="🌌"),
            discord.SelectOption(label="ארגון פשיעה - אוחנה", value="1510196537832116228", emoji="🌌"),
            discord.SelectOption(label="ראש ארגון - לושי", value="1510196537832116226", emoji="🌌"),
            discord.SelectOption(label="יד ימין - לושי", value="1510196537832116225", emoji="🌌"),
            ...
            discord.SelectOption(label="ראש ארגון - נחמן", value="1510196537555161170", emoji="🌌"),
            discord.SelectOption(label="יד ימין - נחמן", value="1510196537530126475", emoji="🌌"),
            discord.SelectOption(label="ארגון פשיעה - נחמן", value="1510196537530126471", emoji="🌌"),
            discord.SelectOption(label="פשע מאורגן", value="1510196537530126469", emoji="🔫"),
            discord.SelectOption(label="ממתין לתדרוך", value="1518220588726882414", emoji="⏳")
        ]
        
        super().__init__(
            placeholder="בחר ארגוני פשיעה וסטטוס תדרוך...",
            min_values=1,
            max_values=len(options),
            options=options,
            custom_id="crime_role_select_pt2",
            row=2
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        for val in self.values:
            if val not in current_pool:
                current_pool.append(val)
        self.parent_view.selected_roles_pool = current_pool
        await interaction.response.send_message("📢 ארגוני פשע סומנו זמנית! לחץ למעלה על 'סיום פנייה והענקת רולים' כדי לנעול ולסגור.", ephemeral=True)
...
class TicketLaunchView(discord.ui.View):
    ...
    async def select_ticket(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        # שמירת האיידי של פותח הטיקט בזיכרון לפי הערוץ החדש שנפתח
        USER_TICKETS[ticket_channel.id] = interaction.user.id
    ...
    async def t_close(self, interaction: discord.Interaction, button: discord.ui.Button):
        class CloseTicketModal(discord.ui.Modal, title="סגירה וארכוב כרטיס תמיכה 🔒"):
            ...
            async def on_submit(self, sub_interaction: discord.Interaction):
                ...
                creator_id = USER_TICKETS.get(sub_interaction.channel.name if False else sub_interaction.channel.id, 0)
...
class MafiaTicketLaunchView(discord.ui.View):
    ...
    async def select_mafia(self, interaction: discord.Interaction, select: discord.ui.Select):
        ...
        stock_text = "💼 בירור כללי - אין ניהםל מלאי ישיר לפנייה זו."
...
class MafiaTicketControlView(discord.ui.View):
    ...
    async def m_update_stock(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        class UpdateStockModal(discord.ui.Modal, title="עדכון מלאי סחורה"):
            ...
            async def on_submit(self, sub_interaction: discord.Interaction):
                try:
                    ...
                    if sub_interaction.view.ticket_type == "weapons":
        ...
        modal_instance = UpdateStockModal()
