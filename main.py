...
class AcceptanceModal(discord.ui.Modal, title="טופס בדיקה וקבלת רולים למשתמש"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # נעילת הנתון בזיכרון הפנימי של הבוט לפי מזהה ההודעה
...
class AcceptanceActionView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if required_role_id not in [role.id for role in interaction.user.roles]:
            await interaction.response.send_message("❌ אין לך הרשאה מתאימה (רול הנהלה עליון) להשתמש ברכיבים אלו!", ephemeral=True)
            return False
        return True
...
class RoleSelectionView(discord.ui.View):
    def __init__(self, target_member: discord.Member, parent_view: AcceptanceActionView):
        ...
        self.add_item(CrimeRoleDropdown(target_member, parent_view))
